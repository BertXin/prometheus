...
var DefaultECSConfig = ECSConfig{
	...
	Limit:           0, // 0 means no limit, discover all instances
}
...
func (d *Discovery) refresh(ctx context.Context) ([]*targetgroup.Group, error) {
	defer d.logger.Debug("ECS discovery completed")
...
func (cl *ecsClient) listTagInstanceIDs(token string, tagFilters []*TagFilter) ([]string, string, error) {
	...
	cl.logger.Debug("get response from ListTagResources.", "response", response)

	tagResources := response.TagResources.TagResource
	if len(tagResources) == 0 { // len(tagResources) = 0 when tagResources = nil
		cl.logger.Debug("ListTagResourcesTagFilter found no resources.", "response", response)
		return []string{}, "", nil
	}
	...
	cl.logger.Debug("listTagResource and get ECS instanceIds. for ListTagResourcesTagFilter.", "instanceIds", resourceIDs)
...
func (cl *ecsClient) getCacheReCheckInstances(cache *targetgroup.Group) []ecs_pop.Instance {
	...
	for tgLabelSetIndex, tgLabelSet := range cache.Targets {
		...
		if pageCount >= MaxPageLimit || tgLabelSetIndex == (len(cache.Targets)-1) {
			...
			if err != nil {
				cl.logger.Error("getCacheReCheckInstances describeInstancesResponse err.", "err", err)
				continue
			}
...
func (cl *ecsClient) listTagInstances(token string, currentTotalCount int, tagFilters []*TagFilter) (string, []ecs_pop.Instance, error) {
	...
	// Only apply limit if cl.limit > 0 (0 means no limit)
	if cl.limit > 0 {
		currentLimit := cl.limit - currentTotalCount // remaining instance count
		if currentLimit >= 0 && currentLimit < pageLimit {
...
func getClient(config *ECSConfig, logger *slog.Logger) (*ecs_pop.Client, error) {
	logger.Debug("Start to get Ecs Client.")
	...
	if config.Policy != "" && config.AccessKey != "" && config.AccessKeySecret != "" && config.RoleArn != "" && config.RoleSessionName != "" {
		logger.Debug("Creating ECS client with RAM role ARN and policy", "role_arn", config.RoleArn)
	...
	if config.RoleSessionName != "" && config.AccessKey != "" && config.AccessKeySecret != "" && config.RoleArn != "" {
		logger.Debug("Creating ECS client with RAM role ARN", "role_arn", config.RoleArn)
	...
	if config.StsToken != "" && config.AccessKey != "" && config.AccessKeySecret != "" {
		logger.Debug("Creating ECS client with STS token", "access_key", config.AccessKey)
	...
	if config.AccessKey != "" && config.AccessKeySecret != "" {
		logger.Debug("Creating ECS client with AccessKey", "access_key", config.AccessKey)
	...
	if config.RoleName != "" {
		logger.Debug("Creating ECS client with ECS RAM role", "role_name", config.RoleName)
	...
	if config.PublicKeyID != "" && config.PrivateKey != "" && config.SessionExpiration != 0 {
		logger.Debug("Creating ECS client with RSA key pair", "public_key_id", config.PublicKeyID)
	...
	logger.Debug("Trying to retrieve client info from RAM metadata")
	...
	if allRoleNameErr != nil {
		logger.Error("Get ECS Client from ram allRoleNameErr.", "err", allRoleNameErr)
	...
	logger.Debug("Start to get Ecs Client from ram2.")

	if roleNameErr != nil {
		logger.Error("Get ECS Client from ram roleNameErr.", "err", roleNameErr)
		return nil, errors.New("aliyun ECS service discovery cant init client, need auth config")
	}
	roleAuth, roleAuthErr := metaData.RamRoleToken(roleName)

	logger.Debug("Start to get Ecs Client from ram3.")

	if roleAuthErr != nil {
		logger.Error("Get ECS Client from ram roleAuthErr.", "err", roleAuthErr)
		return nil, errors.New("aliyun ECS service discovery cant init client, need auth config")
	}
	...
	logger.Debug("Start to get Ecs Client from ram4.")

	if clientErr != nil {
		logger.Error("Get ECS Client from ram clientErr.", "err", clientErr)
		return nil, errors.New("aliyun ECS service discovery cant init client, need auth config")
	}
	return &client, nil
}
...
func addLabel(userID string, port int, instance ecs_pop.Instance) (model.LabelSet, error) {
	...
	for _, tag := range instance.Tags.Tag {
		name := strconv.Quote(tag.TagKey)
		if !model.LabelName(ecsLabelTag+name).IsValid() {
			continue
		}
		labels[model.LabelName(ecsLabelTag+name)] = model.LabelValue(tag.TagValue)
	}

	// Set IP addresses with priority: Private IP > Inner IP > EIP > Public IP
	// This prioritizes internal network addresses for regional deployment
	var addressIP string

	// Check Private IP (highest priority - VPC internal)
	for _, privateIP := range instance.VpcAttributes.PrivateIpAddress.IpAddress {
		if privateIP != "" {
			labels[ecsLabelPrivateIP] = model.LabelValue(privateIP)
			if addressIP == "" {
				addressIP = privateIP
			}
		}
	...
	// Check Inner IP (second priority - Classic network internal)
	for _, innerIP := range instance.InnerIpAddress.IpAddress {
		if innerIP != "" {
			labels[ecsLabelInnerIP] = model.LabelValue(innerIP)
			if addressIP == "" {
				addressIP = innerIP
			}
		}
	...
	// Check EIP (third priority)
	if instance.EipAddress.IpAddress != "" {
		labels[ecsLabelEip] = model.LabelValue(instance.EipAddress.IpAddress)
		if addressIP == "" {
			addressIP = instance.EipAddress.IpAddress
		}
	}

	// Check Public IP (lowest priority)
	for _, publicIP := range instance.PublicIpAddress.IpAddress {
		if publicIP != "" {
	...
	// Set the address label with the highest priority IP
	if addressIP == "" {
		return nil, errors.New("no IP address found for instance")
	}

	addr := net.JoinHostPort(addressIP, strconv.FormatUint(uint64(port), 10))
